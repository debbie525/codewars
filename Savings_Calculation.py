...
def calculate_years(principal, interest_rate, tax_rate, desired_amount):
    continue_calculation = True
    year = 0
    while continue_calculation:
        if principal == desired_amount:
            continue_calculation = False
        else:
            year += 1
            interest = principal * interest_rate 
            tax = interest * tax_rate
            final_interest = interest - tax
            sum = principal + final_interest
            if sum == desired_amount or sum > desired_amount:
                continue_calculation = False
            else:
                principal = sum
            print(f"Sum: {sum}")
    print(f"Years to pay: {year}")    

calculate_years(principal=1000, interest_rate=0.05, tax_rate=0.18, desired_amount=1100)